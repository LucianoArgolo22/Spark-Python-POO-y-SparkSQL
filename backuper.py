...
class CargaAbstracta:
	def __init__(self, tabla, data_base, partition = None):
		'''
	...
	def column_names(self):
		...
		'''
		query = spark.sql(f"select * from {self.data_base}.{self.tabla} limit 0")
	...
	def tablas_DB(self):
		...
		'''
		tablas = spark.sql("show tables in {self.data_base}").collect()
	...
	def partition_table(self, tabla_partition):
		...
		'''
		return spark.sql(f"show partitions {self.data_base}.{tabla_partition}")
...
class ArmadoDeQueries(CargaAbstracta):
	...
	def insert_into_query(self):
		...
		'''
		string = f"""	                insert overwrite table {self.data_base}.{tabla}_bkup_dag 
	...
	def create_table_query(self):
		...
		string = f"""	                 create external table if not exists {self.data_base}.{self.tabla}_bkup_dag 
						...
						partitioned by ({partitions})
						 stored as parquet 
						 location 'hdfs://namesrvprod/path/bkup/bkup_dag/{self.tabla}_bkup_dag'"""
...
class BorradoParticiones(CargaAbstracta):
	...
	def partition_location(self,max_partitions = 3):
		...
		:param max_partitions: The maximum number of partitions to be retained.
		...
		if self.validacion_tiene_particiones(cantidad = max_partitions):
			...
			return "\n".join([f"hdfs dfs -rm -r  /path/bkup/bkup_dag/{self.tabla}_bkup_dag/{list(self.partitions_dict())[0]}={partition[0]}" for partition in filtered_partitions]) 
...
class ComandosBash:
	...
	# 	ls_grep_ComandosBash = " , ".join(self.bash_command(strings="hdfs dfs -ls /path/bkup/bkup_files |grep ComandosBash.sh "))
	...
	def read_write_file(self):
		...
		# 	self.bash_command("hdfs dfs -rm /path/bkup/bkup_files/ComandosBash.sh")
...
if __name__ == "__main__":
	...
	carga_inicial = True if carga_inicial == "True" or carga_inicial == True else False
	...
	for tabla in tablas:
		...
		if carga_inicial:
			tabla = str(tabla)
			tabla_carga_inicial = ArmadoDeQueriesCargaInicial(tabla, data_base)	
			tabla_carga_inicial.sql_sql(max_partitions)

		else:
			tabla = str(tabla)
			tabla_bkup = ArmadoDeQueries(tabla, data_base)
			tabla_bkup.sql_sql()
			delete_partitions = BorradoParticiones(tabla, data_base)
	...
	#bash_generator.append_strings(string_to_write="hdfs dfs -rm /path/bkup/bkup_files/ComandosBash.sh")
	bash_generator.read_write_file()
	bash_generator.bash_command("hdfs dfs -put -f ComandosBash.sh /path/bkup/bkup_files")
