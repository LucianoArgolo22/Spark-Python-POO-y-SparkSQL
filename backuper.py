...
if __name__ == "__main__":
	...
	for tabla in tablas:
		...
		if History_load:
			tabla = str(tabla)
			tabla_History_load = ArmadoDeQueriesCargaInicial(tabla, spark)	
			tabla_History_load.sql_sql(Load)

		else:
			tabla = str(tabla)
			tabla_bkup = ArmadoDeQueries(tabla, spark)
			tabla_bkup.sql_sql()
			delete_partitions = BorradoParticiones(tabla, spark)
	...
	bash_generator.bash_command("hdfs dfs -put -f ComandosBash.sh /santander/bi-corp/bdr/bkup/bkup_files")
